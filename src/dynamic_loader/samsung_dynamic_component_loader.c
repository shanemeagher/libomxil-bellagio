/**
  src/slp_dynamic_component_loader.c

  SLP specific component loader.
...
#include "common.h"
#include "samsung_dynamic_component_loader.h"
 ...
 */
OMX_U32 numLib=0;
static struct BOSA_COMPONENTLOADER *slp_dynamic_loader;

/** @brief The initialization of the SLP specific component loader.
 *
...
void setup_component_loader(BOSA_COMPONENTLOADER * loader) {
	  slp_dynamic_loader = loader;
	  slp_dynamic_loader->BOSA_InitComponentLoader = &BOSA_STE_InitComponentLoader;
	  slp_dynamic_loader->BOSA_DeInitComponentLoader = &BOSA_STE_DeInitComponentLoader;
	  slp_dynamic_loader->BOSA_CreateComponent = &BOSA_STE_CreateComponent;
	  slp_dynamic_loader->BOSA_DestroyComponent = &BOSA_STE_DestroyComponent;
	  slp_dynamic_loader->BOSA_ComponentNameEnum = &BOSA_STE_ComponentNameEnum;
	  slp_dynamic_loader->BOSA_GetRolesOfComponent = &BOSA_STE_GetRolesOfComponent;
	  slp_dynamic_loader->BOSA_GetComponentsOfRole = &BOSA_STE_GetComponentsOfRole;
}

/** @brief the SLP static loader constructor
 *
 * This function creates the SLP dynamic component loader, and creates
 * the list of available components, based on library objects present
 * in the $OMX_COMPONENT_PATH directory.
 */
	...
	if(strncmp(dp->d_name+len-3, ".so", 3) == 0) {
	  ...
	  if((handle = dlopen(lib_absolute_path, RTLD_NOW)) == NULL) {
	    ...
	    if ((fptr = dlsym(handle, "omx_component_library_Setup")) == NULL) {
	      DEBUG(DEB_LEV_ERR, "the library %s is not compatible with SLP dynamic component loader - %s\n", lib_absolute_path, dlerror());
	    } else {
...
/** @brief The destructor of the SLP specific component loader.
 *
...
OMX_ERRORTYPE BOSA_STE_CreateComponent(
  ...
  while(templateList[i]) {
	  ...
	  printf("%s:%d Component: %s\n", __func__, __LINE__, templateList[i]->name);
  ...
  if (componentPosition == -1) {
	  DEBUG(DEB_LEV_ERR, "Component %s not found with current ST dynamic component loader.\n",
		cComponentName);
 ...
 *
 * This function searches in the list of SLP dynamic components and enumerates
 ...
 * standard OMX_GetRolesOfComponent function for the SLP dynamic
 * component loader
 */
 ...
 *
 * This function replicates exactly the behavior of the
 * standard OMX_GetComponentsOfRole function for the SLP dynamic
 * component loader
 */
